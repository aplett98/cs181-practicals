...
# (i.e., the result of parsing an xml file) and return a dictionary mapping
...
def system_call_count_everything(tree):
    """
    arguments:
      tree is an xml.etree.ElementTree object
    returns:
        a dictionary mapping 'num_calls-x' to the amount of times that
        system call x was made.
    """
    c = Counter()
    for el in tree.iter():
        # literally count every single syscall
        c['num_calls-'+el.tag] += 1
    return c
